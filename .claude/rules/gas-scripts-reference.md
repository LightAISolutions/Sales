...
14. **After granting yourself access in the Master ACL, run `clearAllAccessCache` from the editor before trying to sign in again.** `checkSpreadsheetAccess` caches its verdict — grant *and* denial — under `access_<email>` for **600 seconds**, and the cached value is checked before the spreadsheet is ever opened. The bootstrap sequence guarantees a stale denial: `registerSelfProject()` creates the project's ACL column on the first page load and seeds every user whose Role is not `developer` to **FALSE**, so that same load denies you and writes `"0"` to the cache. Ticking TRUE in the sheet afterwards changes nothing until the entry expires, because the read never happens. `clearAllAccessCache()` takes no arguments (it bumps `CACHE_EPOCH` in Script Properties, orphaning every cached entry at once) and is runnable straight from the Run button. **Symptom if skipped:** the auth wall shows `Access denied. Contact your administrator. (code: not_authorized)` while the ACL plainly says TRUE
    - **Diagnosing a denial that survives the flush** — `not_authorized` means `checkSpreadsheetAccess` returned `hasAccess: false`; a *rate-limited* sign-in returns `rate_limited` and an unreadable ACL returns `acl_unavailable`, so those three are distinguishable from the error code alone. Run `inspectCache()` from the editor to see what is actually cached. Note that `auditLog` no-ops while `SPREADSHEET_ID` is still the placeholder, so a project without its own spreadsheet leaves no `login_failed` trail — the Executions tab is the only record
    - **A wrong Role does not deny access, it downgrades it.** If the Role column's value has no matching row in the Master ACL's `Roles` tab, `checkSpreadsheetAccess` falls back to `RBAC_DEFAULT_ROLE` and the user signs in as a viewer. On Classroom that means a successful sign-in with no masthead and no curriculum (viewer is not admitted), which looks like a rendering bug rather than an access one
...
**This is monitored now — check the monitor before re-deriving anything.** `bash scripts/check-acl-health.sh` probes every deployed project that serves `?action=api&op=aclhealth` (currently Receipts and Profiler; the set is discovered from the `.gs` files, so adding the probe elsewhere enrolls it automatically) and answers "can sign-in read the Master ACL right now?" in about two seconds. Exit 0 healthy, 1 unhealthy, 2 nothing-probed — 2 is a failure, not a pass. A daily Routine, **ACL health check (daily)** (`trig_01GeTqB8xp5nG8FCC139Bgr9`, cron `0 10 * * *` = 06:00 ET, fresh session per fire, push + email), runs it and stays silent unless something is wrong. **It deliberately lives outside Google**: a GAS-side time-driven check would sit in the same account whose grant lapses, and `script.scriptapp` is systemically missing on pre-v01.82r projects, so such a trigger could silently never install. The script also reports whether each project's last-known-good ACL snapshot is armed — sign-in can be healthy while the safety net is not, and that combination is what turns the *next* lapse back into a hard lockout.
