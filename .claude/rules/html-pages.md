...
- **html.version.txt is the single source of truth for *what* the current version is.** On page load the polling logic fetches html.version.txt, stores the version as the baseline, creates the version indicator pill, and begins the 10-second polling loop. A version *change* detected mid-session triggers the reload, and after it the page establishes the new version as its baseline, preventing an infinite loop.
- **The `<meta name="build-version">` tag answers a different question, and it is now read on every page.** *(Amended 2026-09-09, v01.85w; promoted from one page to the templates at v05.33w/v05.33r.)* The version pill is fetched with `cache:'no-store'` plus a cache-bust param while the **HTML document itself may be served from cache**, so the pill can report a new version while the JavaScript around it is the old build. The meta tag is the only in-page evidence of which build is actually executing. **Every page** compares the two on the first poll and reloads once when the version file is ahead of the document. The check lives in both HTML templates as of v05.33r, so pages created from them inherit it automatically and no page carries it as a `PROJECT OVERRIDE` any more. **The baseline reload path does not catch this case**: a page opened fresh *after* a deploy sees no version change at all, so nothing fires and a stale document reports itself current. That is exactly how the v01.84w bold fix appeared not to work on a device whose pill already read v01.84w
- **The implementation carries two guards, both load-bearing — preserve them in any propagation.** A **single-attempt loop guard** keyed on the target version in `sessionStorage` — html.version.txt and the meta tag are bumped by hand together under [PC-HTML-VERSION] #2 and *can* drift, and an undetected drift would otherwise reload on every load forever; after one failed attempt the page must fall through and run stale. And a **null-safe read** that disables the check when the meta tag is absent, rather than treating a missing tag as a mismatch. Note that `parseVersionTxt()` strips the leading `v`, so the meta value must be normalised the same way — comparing `v01.85w` against `01.85w` marks every load stale
