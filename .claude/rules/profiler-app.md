...
- **GAS-backed (primary, once deployed)**: the box renders the **Profiler GAS intake app** (`googleAppsScripts/Profiler/Profiler.gs`, embedded as an inline iframe with the dossier's slug prefilled — a `PROJECT OVERRIDE` of the template's full-screen iframe). The developer signs in with Google once (Master-ACL gated), then types the note and/or attaches Word/PDF files (up to 3 × 8 MB) and saves directly in the app. The server validates the session, writes the note verbatim to `Profiler/profiler-notes.json` in the script owner's Drive (`submittedVia: "profiler-intake"`) and any attachments to `Profiler/note-files/<slug>/`. No commit, no deploy — the write is immediate. **Meeting audio** is uploaded browser-side to the user's own Drive with their `drive.file` credential and only its link is stored (`recordingLink`); the backend then files it into `Profiler/meeting-recordings/1-awaiting-transcription/` as `<slug>--YYYY-MM-DD--<original>` (`filerec` op — the `drive.file` scope cannot reach the script's folders, so the browser cannot do this itself). A root sweep on each admin page load rescues strays. The future transcription pass enumerates the queue with `recpending` and moves finished files to `2-transcribed/` with `recdone`. Full layout: PROFILER-SCHEMA.md → Field Notes. File notes get a `[file note: <name> — summary pending triage]` placeholder + `sourceFile` — **triage passes must replace the placeholder with a faithful summary**.
