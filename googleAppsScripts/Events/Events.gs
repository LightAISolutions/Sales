...
var ACL_PAGE_NAME  = "Events";
var PORTAL_ICON    = "📅";
var PORTAL_DESCRIPTION = "The event calendar over the public registry — stars, notes, plans and attendance.";
...
var PROJECT_OVERRIDES = {
  ...
  // PROJECT OVERRIDE: quota discipline (NETWORK-EVENTS-DESIGN-PLAN.md D14) —
  // heartbeat every 600 s instead of the preset's 300 s, and no data poll at
  // all (the registry is a static Pages file; the page fetches starred state
  // on load, on visibilitychange and after writes).
  // Mirrored in Events.html HTML_CONFIG.HEARTBEAT_INTERVAL (600000 ms) —
  // both sides move together per [PC-SESSION-SYNC] #20.
...
// PROJECT: ── Events — E1 scaffold (session 1) ───────────────────────────
...
// a map edit, but only `admin` is admitted: evAdmitted_ turns every other
// tier away at the door with an audit entry (the Network nwAdmitted_ idiom).
// Keep in sync with EV_ROLE_CAPS in Events.html.
var EV_ROLE_CAPS = {
...
// fails closed rather than inheriting a surface.
function evRoleOf_(sess) {
  if ((((sess && sess.permissions) || [])).indexOf('admin') >= 0) return 'admin';
  var r = String((sess && sess.role) || '').toLowerCase().trim();
  return EV_ROLE_CAPS[r] ? r : 'viewer';
}
function evAdmitted_(sess) { return evRoleOf_(sess) === 'admin'; }
function evCan_(sess, cap) {
  if (!evAdmitted_(sess)) return false;
  return (EV_ROLE_CAPS[evRoleOf_(sess)] || []).indexOf(cap) >= 0;
}
// The gate every data op calls after validateSessionForData. Throws rather
// than returning false so a forgotten return value cannot serve gated data.
// Audit details carry the op name and the tier only.
function evRequire_(sess, cap, operationName) {
  if (!evAdmitted_(sess)) {
    auditLog('security_alert', (sess && sess.email) || 'unknown', 'events_not_admitted',
      { operation: operationName || '', role: evRoleOf_(sess) });
    throw new Error('EVENTS_FORBIDDEN');
  }
  if (!evCan_(sess, cap)) {
    auditLog('security_alert', (sess && sess.email) || 'unknown', 'events_capability_denied',
      { operation: operationName || '', role: evRoleOf_(sess), capability: cap });
    throw new Error('EVENTS_FORBIDDEN');
  }
  return true;
}

// PROJECT: ── Enums — EVENTS-SCHEMA.md §5 ─────────────────────────────────
// The Stars row's Attending column; Events.html mirrors it as EV_ATTENDING
// with display labels. A change belongs in all three places in one commit.
var EV_ATTENDING = ['planning', 'registered', 'attended', 'skipped'];
// Event slugs are the public registry's permanent ids (EVENTS-SCHEMA.md §1).
// The page fetches the registry itself; the server only validates the shape
// of the slug a row cites — it never reads the Pages file.
var EV_SLUG_RE = /^[a-z0-9][a-z0-9-]{0,63}$/;

// PROJECT: ── Ids — opaque and random, EVENTS-SCHEMA.md §1 (the Network D8 rule)
// A prefix, a hyphen and 13 base36 characters from 8 random bytes:
// st- Star · pl- Plan · mt- Meeting · pr- Proposed diff. Never a slug, a
// name or a date.
var EV_ID_RE = /^(st|pl|mt|pr)-[0-9a-z]{13}$/;
var EV_ID_PREFIXES = { st: 'stars', pl: 'plans', mt: 'meetings', pr: 'proposed' };

// 8 random bytes → 13 base36 characters. Utilities.getUuid() is the entropy
// source (a v4 UUID carries 122 random bits); it is hashed and the first 8
// bytes of the digest are taken, so no part of the UUID's structure survives.
// 2^64 ≈ 1.8e19 needs 13 base36 digits (36^12 ≈ 4.7e18 < 2^64 < 36^13).
function evRandomBase36_() {
  var digest = Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, Utilities.getUuid());
  // Built in two 32-bit halves to stay inside double precision.
  var hi = 0, lo = 0;
  for (var i = 0; i < 4; i++) hi = hi * 256 + ((digest[i] + 256) % 256);
  for (var j = 4; j < 8; j++) lo = lo * 256 + ((digest[j] + 256) % 256);
  // long division of (hi * 2^32 + lo) by 36, thirteen digits
  var out = '';
  var digits = '0123456789abcdefghijklmnopqrstuvwxyz';
  for (var k = 0; k < 13; k++) {
    var qHi = Math.floor(hi / 36), rHi = hi % 36;
    var mixed = rHi * 4294967296 + lo;
    var qLo = Math.floor(mixed / 36), rem = mixed % 36;
    out = digits.charAt(rem) + out;
    hi = qHi; lo = qLo;
  }
  return out;
}

// New id for a prefix, collision-checked against the tab's first column.
// `takenIds` (a map) lets a batch reserve ids before any row is written.
function evNewId_(prefix, takenIds) {
  var p = String(prefix || '').toLowerCase();
  if (!EV_ID_PREFIXES[p]) throw new Error('EV_BAD_ID_PREFIX');
  var sheet = null;
  try { sheet = ensureEventsTabs_()[EV_ID_PREFIXES[p]]; } catch (eTab) { sheet = null; }
  var existing = {};
  if (sheet) {
  ...
  for (var attempt = 0; attempt < 8; attempt++) {
    var id = p + '-' + evRandomBase36_();
    if (!EV_ID_RE.test(id)) continue;
    if (existing[id] || (takenIds && takenIds[id])) continue;
    if (takenIds) takenIds[id] = true;
    return id;
  }
  throw new Error('EV_ID_COLLISION');
}

// PROJECT: ── Tabs — EVENTS-SCHEMA.md §5, the Receipts in-place header-upgrade idiom
// Idempotent: creates every tab with its frozen header row when missing and
// upgrades headers in place when a column was added after the tab was made.
...
var EV_TABS = {
  stars: ['Stars', ['Star ID', 'Owner', 'Event Slug', 'Note', 'Attending', 'Created At', 'Updated At']],
  plans: ['Plans', ['Plan ID', 'Owner', 'Event Slug', 'Day', 'Items', 'Narrative Link', 'Created At', 'Updated At']],
  meetings: ['Meetings', ['Meeting ID', 'Owner', 'Event Slug', 'Contact ID', 'Account ID', 'Start', 'End', 'Place',
    'Note', 'ICS UID', 'Network Interaction ID', 'Created At']],
...
function ensureEventsTabs_() {
  if (!SPREADSHEET_ID || SPREADSHEET_ID === 'YOUR_SPREADSHEET_ID') throw new Error('SPREADSHEET_NOT_CONFIGURED');
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  var out = {};
  for (var key in EV_TABS) {
    if (!EV_TABS.hasOwnProperty(key)) continue;
    var name = EV_TABS[key][0], headers = EV_TABS[key][1];
    ...
    if (sh.getLastRow() === 0) {
      ...
      // In-place header upgrade — a header cell that differs from the schema
      // is rewritten; cells beyond the current width are appended. Data rows
      // are never touched.
      var width = Math.max(sh.getLastColumn(), 1);
      var current = sh.getRange(1, 1, 1, width).getValues()[0];
      for (var c = 0; c < headers.length; c++) {
        if (String(current[c] || '') !== headers[c]) sh.getRange(1, c + 1).setValue(headers[c]);
      }
...
// PROJECT: ── Ownership — copied verbatim from Receipts.gs via Network.gs (the widening path, D7)
// Sharing — the Shares tab holds one row per grant (Owner → Grantee, scope
// "view" or "edit"). Dormant in v1 (no UI — EVENTS-SCHEMA.md §2) but the
// scoping runs, so a later widening is a map edit plus a panel, not a
// re-architecture. Not-found-not-forbidden: an op on another owner's row
// answers as if the row did not exist, so existence never leaks.

// Latest grant scope from owner → grantee ('', 'view', or 'edit').
function getShareScope_(ownerEmail, granteeEmail) {
  var sh = ensureEventsTabs_().shares;
...
// PROJECT: ── Unauthenticated probes (ported verbatim from Network.gs at N0) — GET ?action=api&op=aclhealth and op=quota
// aclhealth: ported from Receipts/Profiler so scripts/check-acl-health.sh enrols
// this project automatically (it discovers probes from the .gs files). Runs the
// exact read sequence checkSpreadsheetAccess performs before it trusts the
// list and reports which stage failed. Safe without auth: only reason codes
// the sign-in screen already shows, never emails, rows or ACL contents; the
// caught exception's message has the spreadsheet id redacted. 60-second cache.
...
// PROJECT: ── op=quota — copied verbatim from Network.gs (the region N0 defined
// and Q0 rolled to every project; scripts/check-quota.sh reads it).
...
// PROJECT: ── Events ops (action=events, eop=list|star|unstar|note) ───────
// Over the same cookie-less fetch transport as Network's ops, with a GET
// mirror on action=api&op=events. Order of checks: session → the app door
// (evRequire_ — admin only under D7; every other tier is turned away with an
// audit entry before any tab is opened) → the op. E1 session 1 ships the
// Stars ops only; the registry itself is a public Pages file the page reads
// directly and this script never touches (EVENTS-SCHEMA.md §3).
function handleEventsOp_(e) {
  var p = (e && e.parameter) || {};
  var op = String(p.eop || '');
  ...
  var sess = null;
  try {
    sess = validateSessionForData(session, 'events_' + op);
    if (op === 'list') {
      evRequire_(sess, 'calendar', 'events_list');
      var scope = resolveOwnerSet_(sess, p.owner || '');
      if (scope.error) return { success: false, error: scope.error };
      var tabs = ensureEventsTabs_();
      // Minimum-necessary: the star id, the slug it points at, the attending
      // state and the timestamp — plus the owner's own note, because the
      // detail sheet shows it and a per-open detail op would cost an
      // execution each time under D14. Nothing about people is here.
      var stars = evListRows_(tabs.stars, scope.set, {
        id: 'Star ID', slug: 'Event Slug', attending: 'Attending', note: 'Note', updatedAt: 'Updated At' });
      auditLog('data_read', sess.email, 'events_list', { stars: stars.length });
      return { success: true, role: evRoleOf_(sess), caps: EV_ROLE_CAPS[evRoleOf_(sess)] || [], stars: stars };
    }
    if (op === 'star' || op === 'unstar' || op === 'note') {
    ...
    if (msg === 'EVENTS_FORBIDDEN') return { success: false, error: 'ROLE_DENIED', role: evRoleOf_(sess) };
    return { success: false, error: msg };
  }
}

// Rows of a tab as objects of the requested columns, scoped to the owner set
// and with soft-deleted rows dropped where the tab has a Deleted At column.
// `pick` maps output key → header name.
function evListRows_(sheet, ownerSet, pick) {
  var last = sheet.getLastRow(), width = sheet.getLastColumn();
  if (last < 2 || width < 1) return [];
  var vals = sheet.getRange(1, 1, last, width).getValues();
  var headers = vals[0], idx = {};
  for (var h = 0; h < headers.length; h++) idx[String(headers[h])] = h;
  var ownerCol = idx['Owner'], delCol = idx['Deleted At'];
  var out = [];
  for (var r = 1; r < vals.length; r++) {
    var row = vals[r];
    var owner = String(row[ownerCol] || '').toLowerCase();
    if (!ownerSet[owner]) continue;
    if (delCol !== undefined && String(row[delCol] || '')) continue;
    var o = { owner: owner };
    for (var k in pick) {
      if (!pick.hasOwnProperty(k)) continue;
      var ci = idx[pick[k]];
      var v = ci === undefined ? '' : row[ci];
...
function evStr_(v, max) { var s = (v == null) ? '' : String(v).trim(); return max ? s.slice(0, max) : s; }
function evCell_(v) { return (v instanceof Date) ? v.toISOString() : String(v == null ? '' : v); }

// The Stars write path (EVENTS-SCHEMA.md §5). One row per owner × slug:
//   star   — create the row (Attending defaults to `planning`), or set
//            Attending on the existing one when the body carries it
//   unstar — remove the row (Stars has no Deleted At column; a star is not a
//            record about a person, so a hard delete is the schema's choice)
//   note   — write Note and/or Attending on the row, creating it when the
//            event was not yet starred (a note implies interest)
// Every field is validated before the write: the slug against EV_SLUG_RE
// (never against the registry — the page owns that), Attending against the
// enum, the note trimmed and bounded. Body-POST-capable (the note rides in
// the body); scoped to the session user's own rows (resolveOwnerScope_ with
// needEdit); audited with ids and counts only.
function evStarOp_(sess, p, mode) {
  ...
  var attending = null;
  if (p.attending !== undefined && p.attending !== null && String(p.attending) !== '') {
    attending = evStr_(p.attending, 20).toLowerCase();
    if (EV_ATTENDING.indexOf(attending) < 0) return { success: false, error: 'bad_attending' };
  }
  var hasNote = (mode === 'note') && p.note !== undefined && p.note !== null;
  var note = hasNote ? evStr_(p.note, 2000) : '';
  var sheet = ensureEventsTabs_().stars;
  var last = sheet.getLastRow(), width = Math.max(sheet.getLastColumn(), 1);
  var headers = sheet.getRange(1, 1, 1, width).getValues()[0], idx = {};
  for (var h = 0; h < headers.length; h++) idx[String(headers[h])] = h;
  var found = -1, row = null;
  if (last > 1) {
    var vals = sheet.getRange(2, 1, last - 1, width).getValues();
    for (var r = 0; r < vals.length; r++) {
      if (String(vals[r][idx['Owner']] || '').toLowerCase() === owner
          && String(vals[r][idx['Event Slug']] || '') === slug) { found = r + 2; row = vals[r]; break; }
    }
  }
  var now = new Date().toISOString();
  if (mode === 'unstar') {
    if (found < 0) return { success: true, removed: false, slug: slug };
    var goneId = String(row[idx['Star ID']] || '');
    sheet.deleteRow(found);
    auditLog('data_write', sess.email, 'events_unstar', { starId: goneId, slug: slug });
    return { success: true, removed: true, slug: slug };
  }
  if (found < 0) {
    var id = evNewId_('st');
    var fresh = [id, owner, slug, note, attending || 'planning', now, now];
    sheet.appendRow(fresh);
    auditLog('data_write', sess.email, 'events_' + mode, { starId: id, slug: slug, created: 1, noteLength: note.length });
    return { success: true, created: true,
             star: { id: id, slug: slug, attending: fresh[4], note: note, updatedAt: now } };
  }
  if (hasNote) sheet.getRange(found, idx['Note'] + 1).setValue(note);
  if (attending) sheet.getRange(found, idx['Attending'] + 1).setValue(attending);
  sheet.getRange(found, idx['Updated At'] + 1).setValue(now);
  var starId = String(row[idx['Star ID']] || '');
  auditLog('data_write', sess.email, 'events_' + mode,
    { starId: starId, slug: slug, created: 0, noteLength: hasNote ? note.length : -1 });
  return { success: true, created: false,
           star: { id: starId, slug: slug, attending: attending || evCell_(row[idx['Attending']]),
                   note: hasNote ? note : evCell_(row[idx['Note']]), updatedAt: now } };
}
// PROJECT END
// ══════════════
...
function doPost(e) {
  ...
  // PROJECT: Events ops via fetch() — same transport rationale as the fetch
  // routes above; session + door (admin-only, D7) gated in handleEventsOp_.
  if (action === "events") {
    return ContentService.createTextOutput(JSON.stringify(handleEventsOp_(e)))
      .setMimeType(ContentService.MimeType.JSON);
  }
...
function doGet(e) {
  ...
  // aclHealthProbe_). Enrols this project in scripts/check-acl-health.sh.
  ...
  if (action === 'api') {
    ...
    try {
      ...
      } else if (apiOp === 'events') {
        // PROJECT: GET mirror of the Events ops (small params only)
        apiResult = handleEventsOp_(e);
      } else {
