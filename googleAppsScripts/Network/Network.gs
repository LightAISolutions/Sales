...
var SPREADSHEET_ID = "1YjY3XMXDGwhW4U-lf3aKxGTJz5JvMyQcdCyVWdptBiQ";
...
var ACL_PAGE_NAME  = "Network";
var PORTAL_ICON    = "📇";
var PORTAL_DESCRIPTION = "Business-card capture and the account-centric contact layer.";
...
var PROJECT_OVERRIDES = {
  ...
  // all (the page fetches on load, on visibilitychange and after writes).
  // Mirrored in Network.html HTML_CONFIG.HEARTBEAT_INTERVAL (600000 ms) —
...
// a map edit, but only `admin` is admitted: nwAdmitted_ turns every other
// tier away at the door with an audit entry (the Classroom clAdmitted_ idiom).
// Keep in sync with NW_ROLE_CAPS in Network.html.
var NW_ROLE_CAPS = {
...
function nwRoleOf_(sess) {
  if ((((sess && sess.permissions) || [])).indexOf('admin') >= 0) return 'admin';
  var r = String((sess && sess.role) || '').toLowerCase().trim();
  return NW_ROLE_CAPS[r] ? r : 'viewer';
}
function nwAdmitted_(sess) { return nwRoleOf_(sess) === 'admin'; }
function nwCan_(sess, cap) {
  if (!nwAdmitted_(sess)) return false;
  return (NW_ROLE_CAPS[nwRoleOf_(sess)] || []).indexOf(cap) >= 0;
}
...
// Audit details carry the op name and the tier only — never a card field
// (NETWORK-SCHEMA.md §12).
function nwRequire_(sess, cap, operationName) {
  if (!nwAdmitted_(sess)) {
    auditLog('security_alert', (sess && sess.email) || 'unknown', 'network_not_admitted',
      { operation: operationName || '', role: nwRoleOf_(sess) });
    throw new Error('NETWORK_FORBIDDEN');
  }
  if (!nwCan_(sess, cap)) {
    auditLog('security_alert', (sess && sess.email) || 'unknown', 'network_capability_denied',
      { operation: operationName || '', role: nwRoleOf_(sess), capability: cap });
    throw new Error('NETWORK_FORBIDDEN');
  }
  return true;
}

// PROJECT: ── Enums (D5) — flat server lists, NETWORK-SCHEMA.md §4 ────────
// The extraction responseSchema (N1) and the save-time validator read these;
// Network.html mirrors them as the grouped NW_ENUMS map with display labels.
// A change to a value belongs in all three places in one commit.
var NW_RELATIONSHIPS = ['customer', 'partner', 'supplier', 'target', 'competitor', 'channel', 'other'];
var NW_STAGES = ['none', 'prospecting', 'discovery', 'rfp', 'shortlist', 'negotiation', 'post-award', 'won', 'lost'];
var NW_ROLES = ['decision-maker', 'economic-buyer', 'technical-evaluator', 'point-of-contact', 'champion',
                'influencer', 'gatekeeper', 'procurement', 'peer', 'other'];
var NW_INTERACTION_KINDS = ['scan', 'meeting', 'call', 'email-out', 'email-in', 'calendar', 'note', 'linkedin',
                            'account-change', 'merge'];
var NW_SIGNAL_KINDS = ['exhibitor', 'speaker', 'press-release', 'press-quote', 'newsroom', 'agenda', 'docket',
                       'luma', 'linkedin-manual', 'registrant-mail', 'directory', 'other'];
var NW_CONSENT = ['yes', 'no', 'unknown'];
var NW_DRAFT_STATUS = ['draft', 'sent', 'discarded'];
var NW_SIGNAL_SOURCES = ['events', 'scraper', 'manual'];
// `stage` may be anything other than `none` only for these relationships
// (the D5 validator rule); the save path enforces it in N1/N2.
var NW_STAGE_RELATIONSHIPS = ['target', 'customer'];

// PROJECT: ── Ids (D8) — opaque and random, NETWORK-SCHEMA.md §1 ──────────
// A prefix, a hyphen and 13 base36 characters from 8 random bytes. Never a
// name, an email, a company or a date (§164.514(c)); the Receipts
// Store_Name-YYYYMMDD pattern is NOT transferable to people.
var NW_ID_RE = /^[acisdm]-[0-9a-z]{13}$/;
var NW_ID_PREFIXES = { a: 'Accounts', c: 'Contacts', i: 'Interactions', s: 'Signals', d: 'Drafts', m: 'Mailings' };
...
function nwNewId_(prefix, takenIds) {
  var p = String(prefix || '').toLowerCase();
  if (!NW_ID_PREFIXES[p]) throw new Error('NW_BAD_ID_PREFIX');
  var sheet = null;
  try { sheet = ensureNetworkTabs_()[NW_ID_PREFIXES[p].toLowerCase()]; } catch (eTab) { sheet = null; }
  var existing = {};
  if (sheet) {
  ...
  for (var attempt = 0; attempt < 8; attempt++) {
    var id = p + '-' + nwRandomBase36_();
    if (!NW_ID_RE.test(id)) continue;
    if (existing[id] || (takenIds && takenIds[id])) continue;
    if (takenIds) takenIds[id] = true;
    return id;
  }
  throw new Error('NW_ID_COLLISION');
}

// PROJECT: ── Tabs — NETWORK-SCHEMA.md §3, the Receipts in-place header-upgrade idiom
...
// spreadsheet. Column order is the schema's, verbatim — a change belongs in
// NETWORK-SCHEMA.md §3 first.
var NW_TABS = {
  accounts: ['Accounts', [
    'Account ID', 'Owner', 'Name', 'Normalised Name', 'Domain', 'Profiler Slug', 'Relationship', 'Stage',
    'Segment IDs', 'Tags', 'HQ', 'Newsroom URL', 'Notes', 'Created At', 'Updated At', 'Deleted At']],
  contacts: ['Contacts', [
    'Contact ID', 'Owner', 'Account ID', 'Full Name', 'First', 'Last', 'Title', 'Department', 'Role',
    'Emails', 'Phones', 'Address', 'LinkedIn', 'Website', 'Socials', 'Languages', 'Source Event', 'Met Date',
    'Consent Marketing', 'Do Not Contact', 'Tags', 'Notes', 'Card Front Link', 'Card Back Link',
    'Raw Extraction', 'Created At', 'Updated At', 'Deleted At']],
  interactions: ['Interactions', [
    'Interaction ID', 'Owner', 'Contact ID', 'Account ID', 'Kind', 'Date', 'Summary', 'Evidence Link',
    'Event Slug', 'Created At']],
  signals: ['Signals', [
    'Signal ID', 'Owner', 'Account ID', 'Contact ID', 'Event Slug', 'Kind', 'Person Name', 'Person Title',
    'Evidence URL', 'First Seen', 'Last Seen', 'Confidence', 'Note', 'Source']],
  mailings: ['Mailings', [
    'Mailing ID', 'Owner', 'Template Name', 'Subject Template', 'Body Template', 'Filter', 'Created At']],
  drafts: ['Drafts', [
...
function ensureNetworkTabs_() {
  ...
  var out = {};
  for (var key in NW_TABS) {
    if (!NW_TABS.hasOwnProperty(key)) continue;
    var name = NW_TABS[key][0], headers = NW_TABS[key][1];
...
// The dedupe key for company names (NETWORK-SCHEMA.md §3 — Normalised Name):
...
function nwNormaliseCompany_(name) {
  return String(name || '').toLowerCase()
    .replace(/&/g, ' and ')
    .replace(NW_LEGAL_SUFFIX_RE, ' ')
    .replace(/[^a-z0-9À-ɏ぀-ヿ㐀-鿿가-힯 ]+/g, ' ')
    .replace(/\s+/g, ' ').trim();
}

// PROJECT: ── Drive folder registry (NETWORK-SCHEMA.md §6; the Profiler recfolders precedent)
// The `Network App/` tree is created BROWSER-side with the signed-in user's
// own drive.file credential, so it lives in their Drive and not in the Drive
// of whichever account owns this script (DriveApp here acts as the deploying
// account and must not be used for these files). The browser cannot re-find
// a drive.file folder it created in an earlier session, so the ids are parked
// here: `root` (Network App), `inbox` (_inbox), and `accounts` — a map of
// Account ID → folder id, filled as <Company>/ folders are created on save.
var PROP_NW_FOLDERS = 'NW_FOLDER_IDS';

function nwFoldersGet_() {
  var raw = PropertiesService.getScriptProperties().getProperty(PROP_NW_FOLDERS);
  if (!raw) return null;
  try { return JSON.parse(raw); } catch (err) { return null; }
}

// Drive file ids are opaque strings; the only shape check that is safe to
// apply is "non-empty, no whitespace, sane length".
function nwFolderIdOk_(v) { return /^[A-Za-z0-9_-]{10,128}$/.test(String(v || '')); }

function nwFoldersSet_(ids) {
  var prev = nwFoldersGet_() || { root: '', inbox: '', accounts: {} };
  var clean = {
    root: String((ids && ids.root) || prev.root || ''),
    inbox: String((ids && ids.inbox) || prev.inbox || ''),
    accounts: (prev.accounts && typeof prev.accounts === 'object') ? prev.accounts : {}
  };
  if (!nwFolderIdOk_(clean.root) || !nwFolderIdOk_(clean.inbox)) throw new Error('INVALID_INPUT');
  var add = ids && ids.accounts;
  if (add && typeof add === 'object') {
    for (var k in add) {
      if (!add.hasOwnProperty(k)) continue;
      if (!/^a-[0-9a-z]{13}$/.test(k) || !nwFolderIdOk_(add[k])) throw new Error('INVALID_INPUT');
      clean.accounts[k] = String(add[k]);
    }
  }
  PropertiesService.getScriptProperties().setProperty(PROP_NW_FOLDERS, JSON.stringify(clean));
  return clean;
}
...
function getShareScope_(ownerEmail, granteeEmail) {
  var sh = ensureNetworkTabs_().shares;
...
// PROJECT: ── Unauthenticated probes — GET ?action=api&op=aclhealth and op=quota
...
function handleNetworkOp_(e) {
  ...
  try {
    ...
    if (op === 'folders') {
      ...
      return { success: true, folders: nwFoldersGet_() };
    }
    if (op === 'setfolders') {
      nwRequire_(sess, 'contacts', 'network_setfolders');
      var accountsMap = null;
      if (p.accounts) { try { accountsMap = JSON.parse(String(p.accounts)); } catch (eJ) { return { success: false, error: 'bad_folder_ids' }; } }
      try {
        var saved = nwFoldersSet_({ root: p.root, inbox: p.inbox, accounts: accountsMap });
        auditLog('data_write', sess.email, 'network_folders_registered',
    ...
    if (msg === 'NETWORK_FORBIDDEN') return { success: false, error: 'ROLE_DENIED', role: nwRoleOf_(sess) };
    return { success: false, error: msg };
  }
}

// Rows of a tab as objects of the requested columns, scoped to the owner set
// and with soft-deleted rows dropped. `pick` maps output key → header name.
function nwListRows_(sheet, ownerSet, pick) {
...
function doPost(e) {
  ...
  // PROJECT: Network ops via fetch() — same transport rationale as the fetch
  // routes above; session + door (admin-only, D7) gated in handleNetworkOp_.
  if (action === "network") {
    return ContentService.createTextOutput(JSON.stringify(handleNetworkOp_(e)))
      .setMimeType(ContentService.MimeType.JSON);
  }
...
function doGet(e) {
  ...
  if (action === 'api') {
    ...
    try {
      ...
      } else if (apiOp === 'network') {
        // PROJECT: GET mirror of the Network ops (small params only)
        apiResult = handleNetworkOp_(e);
      } else {
