...
// PROJECT: ── Industry Guidance (Classroom-hosted since C3 session 3) ─────
// The nine document-analysis modules, `guidanceDocs_`/`guidanceIndex_`/
// `guidanceDoc_` and the `gop=index|doc` read ops they answered were copied
// into `Classroom.gs` at C3 session 1 and DELETED from here at session 3
// (PHASE6-CLASSROOM-DESIGN.md → Phase plan → C3 slice plan). Guidance is both
// hosted and studied in Classroom now: the masthead button and the dossier
// mention chips deep-link to `Classroom.html#guidance/<id>`, and the
// per-account section ticks went with them (exportGuidanceProgress() below is
// the one-shot export that carried them).
//
// Two guidance ops still answer here, and neither serves module content:
//   gop=mentions             — the dossier's "✦ Covered in guidance modules"
//                              line, relayed from Classroom's peer route.
//   gop=progress|setprogress — the reading-progress store, which is no longer
//                              a guidance store at all: what is left in it is
//                              `study-<slug>` and `dossier-<slug>` ticks.
// `guidanceAllowed_(sess)` is still the boundary for both. The mentions answer
// names modules, so the tier that may see module titles has not moved; and the
// progress ops were always gated at exactly this tier.
function handleGuidanceOp_(e) {
  ...
  try {
    ...
    // 'index' and 'doc' land here deliberately: a browser still holding the
    // pre-C3 page would otherwise get a silent empty library rather than a
    // legible refusal, and the page's own error path renders the reason.
...
// PROJECT: ── Reading progress (server-side, per account) ───────────────
// gop=progress / gop=setprogress — the same role gate the guidance ops used.
// The account's section ticks live server-side so progress follows the account
// across devices; the page keeps localStorage as its offline fallback and
// migrates local-only ticks up in one batch on its first successful sync.
// Storage is one Script Property per account ('gd_progress:<email>' →
// {docId:{secId:'YYYY-MM-DD'}}): the blobs are tiny (well under the 9KB/value
// cap) and have no cross-project consumers, so PropertiesService beats the
// Master ACL spreadsheet pattern here — no spreadsheet round-trip per tick.
// Doc and section ids are validated before they are stored, so junk params can
// never grow the store.
//
// **Since C3 session 3 this is no longer a guidance store.** Guidance module
// ticks were exported to Classroom's `cl_progress:<email>` in one shot (see
// the export block below) and what remains here is `study-<slug>` and
// `dossier-<slug>` — both slug-keyed public Pages content, both validated by
// containment. The name of the property is unchanged deliberately: renaming it
// would orphan every study and dossier tick in it to buy a tidier label.
//
...
//
// Study-guide progress (Phase 5, v01.29g): doc ids 'study-<slug>'. Dossier
// progress (v01.34g) adds 'dossier-<slug>' on the same rule, so marking a
// dossier read is per-account and cross-device like everything else here. A
// public Pages study guide's section ids aren't known server-side, so their
// validation is containment rather than enumeration: the slug must exist in
// the public registry (cached 6h), section ids must look like the renderer's
// kebab-case anchors, and each doc's tick set is capped — junk params still
// can't grow the store meaningfully. Tiers outside the guidance role gate
// never reach these ops; their study ticks stay in the page's localStorage
// fallback.
...
function gdProgressWrite_(sess, p) {
  ...
  try {
    ...
    var studySlugs = null;   // registry fetched only when a slug doc id appears
    for (var docId in delta) {
      if (!delta.hasOwnProperty(docId)) continue;
      // ONE validation regime since C3 session 3. Guidance modules used to be
      // validated by ENUMERATION against guidanceDocs_(); that registry now
      // lives in Classroom.gs and a module tick belongs in `cl_progress:`, so
      // every id this store still accepts is slug-keyed public Pages content —
      // 'study-<slug>' and 'dossier-<slug>' — validated by CONTAINMENT: the
      // slug must exist in the registry, ids must look like the renderer's
      // kebab-case anchors, and each doc's tick set is capped. A module id
      // arriving here (a cached pre-C3 page, a stale localStorage batch) is
      // simply skipped, which is the same answer an unknown id always got.
      var slugPrefix = docId.indexOf('study-') === 0 ? 'study-'
...
// PROJECT: ── Guidance progress export (C3 session 3, one-shot) ────────────
// The migration's export half. `gd_progress:<email>` is a SHARED store — one
// Script Property per account holding guidance module ticks, `study-<slug>`
// ticks and `dossier-<slug>` ticks together — so this is a FILTER, never a
// property move. Only ids in GD_MIGRATED_MODULE_IDS leave; study-guide and
// dossier ticks stay exactly where they are, because nothing in Classroom
// reads them and moving them would lose them at the next write on either side.
//
// The id list is a frozen literal rather than a walk of guidanceDocs_(),
// which no longer exists in this project. That is the honest source anyway:
// module ids are permanent by design — Scraper's `guidance:<module-id>` seed
// sources and every Classroom lesson's provenance stamp key on them — so the
// literal is the same list the registry would have produced, and it outlives
// the content it used to sit beside.
//
// Run order, all from the Apps Script editor (these are the no-underscore
// names, so they appear in the Run dropdown):
//   1. Profiler   exportGuidanceProgress()             — read-only; copy the JSON
//   2. Classroom  (paste JSON into GUIDANCE_PROGRESS_IMPORT Script Property)
//   3. Classroom  previewGuidanceProgressImport()      — dry run, writes nothing
//   4. Classroom  applyGuidanceProgressImport()        — the import
//   5. Classroom  verifyGuidanceProgressImport()       — export vs live store
//   6. Profiler   previewGuidanceProgressPrune()       — dry run
//   7. Profiler   applyGuidanceProgressPrune()         — drops the migrated ids
// Steps 6-7 are OPTIONAL and must not run before step 5 reports clean: study
// history is the developer's own, not resettable state, and a prune is the
// only irreversible step in the sequence.
var GD_MIGRATED_MODULE_IDS = [
  'bess-tech-fundamentals-2026-08',
  'power-infra-aidc-2026-08',
  'nvidia-800vdc-2026-08',
  'utility-aidc-procurement-2026-08',
  'large-load-interconnection-2026-09',
  'grid-equipment-shortage-2026-09',
  'china-policy-stack-2026-08',
  'bess-bankability-2026-08',
  'eo14420-bulk-power-2026-08'
];

function gdMigratedIdSet_() {
  var ids = {};
  for (var i = 0; i < GD_MIGRATED_MODULE_IDS.length; i++) ids[GD_MIGRATED_MODULE_IDS[i]] = true;
  return ids;
}

// Every `gd_progress:` property, narrowed to the migrated module ids. Pure —
// it reads properties and returns a value, so the export and the prune's dry
// run answer from exactly the same derivation.
function gdGuidanceProgressScan_() {
  var ids = gdMigratedIdSet_();
  var props = PropertiesService.getScriptProperties().getProperties();
  var out = { exportedAt: new Date().toISOString(),
              source: 'Profiler.gs ' + VERSION + ' ' + GD_PROGRESS_PROP_PREFIX,
              moduleIds: GD_MIGRATED_MODULE_IDS.slice(),
              accounts: {},
              counts: { accounts: 0, modules: 0, ticks: 0, keptDocs: 0 } };
  for (var key in props) {
    if (!props.hasOwnProperty(key) || key.indexOf(GD_PROGRESS_PROP_PREFIX) !== 0) continue;
    var acct = key.slice(GD_PROGRESS_PROP_PREFIX.length);
    if (!acct) continue;
    var stored = {};
    try { stored = JSON.parse(props[key] || '{}') || {}; } catch (pe) { continue; }
    var take = {}, took = 0, docs = 0;
    for (var docId in stored) {
      if (!stored.hasOwnProperty(docId)) continue;
      if (!ids[docId]) { out.counts.keptDocs++; continue; }   // study-/dossier- stay here
      var secs = stored[docId] || {}, keep = {}, n = 0;
      for (var secId in secs) {
        // A falsy value is not a tick — gdProgressWrite_ deletes rather than
        // storing false, so anything falsy here is corruption, not history.
        if (!secs.hasOwnProperty(secId) || !secs[secId]) continue;
        keep[secId] = secs[secId];   // 'YYYY-MM-DD', or legacy true, verbatim
        n++;
      }
      if (!n) continue;
      take[docId] = keep; took += n; docs++;
    }
    if (!took) continue;
    out.accounts[acct] = take;
    out.counts.accounts++; out.counts.modules += docs; out.counts.ticks += took;
  }
  return out;
}

// STEP 1 — read-only. Mutates nothing, so it is safe to run as often as you
// like, and the JSON it prints is the pre-migration export the Classroom side
// is verified against (PHASE6-CLASSROOM-DESIGN.md → Verification expectations).
function exportGuidanceProgress() {
  var out = gdGuidanceProgressScan_();
  Logger.log('guidance progress export — %s account(s), %s module(s), %s tick(s); %s non-module doc(s) staying here',
    out.counts.accounts, out.counts.modules, out.counts.ticks, out.counts.keptDocs);
  Logger.log(JSON.stringify(out));
  return out;
}

// STEPS 6-7 — the prune. Drops ONLY the migrated module ids from each account's
// property and leaves every `study-<slug>` and `dossier-<slug>` tick untouched.
// An account left with nothing but module ticks has its property deleted
// outright rather than left holding '{}'.
function gdPruneGuidanceProgress_(apply) {
  var ids = gdMigratedIdSet_();
  var props = PropertiesService.getScriptProperties();
  var all = props.getProperties();
  var plan = { apply: !!apply, accounts: [], docsDropped: 0, ticksDropped: 0, propsDeleted: 0 };
  for (var key in all) {
    if (!all.hasOwnProperty(key) || key.indexOf(GD_PROGRESS_PROP_PREFIX) !== 0) continue;
    var stored = {};
    try { stored = JSON.parse(all[key] || '{}') || {}; } catch (pe) { continue; }
    var dropped = [], ticks = 0;
    for (var docId in stored) {
      if (!stored.hasOwnProperty(docId) || !ids[docId]) continue;
      dropped.push(docId);
      ticks += Object.keys(stored[docId] || {}).length;
    }
    if (!dropped.length) continue;
    for (var d = 0; d < dropped.length; d++) delete stored[dropped[d]];
    var emptied = !Object.keys(stored).length;
    plan.accounts.push({ account: key.slice(GD_PROGRESS_PROP_PREFIX.length),
                         dropped: dropped, ticks: ticks,
                         propertyDeleted: emptied, remaining: Object.keys(stored) });
    plan.docsDropped += dropped.length; plan.ticksDropped += ticks;
    if (emptied) plan.propsDeleted++;
    if (apply) {
      if (emptied) props.deleteProperty(key);
      else props.setProperty(key, JSON.stringify(stored));
    }
  }
  Logger.log('%s guidance progress prune — %s doc(s) / %s tick(s) across %s account(s)',
    apply ? 'APPLIED' : 'DRY RUN', plan.docsDropped, plan.ticksDropped, plan.accounts.length);
  Logger.log(JSON.stringify(plan));
  return plan;
}

function previewGuidanceProgressPrune() { return gdPruneGuidanceProgress_(false); }
function applyGuidanceProgressPrune() { return gdPruneGuidanceProgress_(true); }
