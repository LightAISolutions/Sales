...
function doPost(e) {
  ...
  // ── Fetch-transport auth (iframe-free architecture) ──
  // Google began serving X-Frame-Options: SAMEORIGIN on framed /exec responses
  // (2026-07-09), which broke the hidden-iframe postMessage exchange in every
  // browser context. These actions expose the existing session functions over
  // ContentService JSON so the HTML layer can run the entire auth lifecycle
  // via fetch() POSTs — fetch is unaffected by framing policy.
  if (action === "exchangeToken") {
    var xtToken = (e && e.parameter && e.parameter.token) || "";
    var xtResult;
    try {
      // The iframe flow auto-created HMAC_SECRET via doGet page loads; the fetch
      // flow enters through doPost, so ensure required Script Properties here too.
  ...
  // PROJECT START — testauthgas1 fetch-transport data operations
  // Wraps the google.script.run data functions for the HTML-layer Live Data app.
  // validateSessionForData throws on bad sessions — caught and returned as {error}.
  if (action === "dataOp") {
    var dpoToken = (e && e.parameter && e.parameter.token) || "";
    var dpoOp = (e && e.parameter && e.parameter.op) || "";
    var dpoResult;
    try {
      if (dpoOp === "getAll") {
        dpoResult = { type: 'live-data', data: getAuthenticatedData(dpoToken) };
      } else if (dpoOp === "writeCell") {
        dpoResult = writeCell(dpoToken, parseInt(e.parameter.row, 10), parseInt(e.parameter.col, 10), e.parameter.value || "");
      } else if (dpoOp === "addRow") {
        dpoResult = addRow(dpoToken, e.parameter.values || "[]");
      } else if (dpoOp === "deleteRow") {
        dpoResult = deleteRow(dpoToken, e.parameter.rowIndex);
      } else if (dpoOp === "presence") {
        dpoResult = updatePresence(dpoToken, (e.parameter.state || ""));
      } else if (dpoOp === "getAppUi") {
        // Hybrid architecture: session-gated app UI delivery — the UI ships as
        // content (css+js strings), so framing policy can never block it and
        // the markup never appears in the public Pages HTML
        var uiCache = getEpochCache();
        if (!dpoToken || !uiCache.get("session_" + dpoToken)) {
          dpoResult = { error: "no_session" };
        } else {
          dpoResult = getAppUiPayload_();
        }
      } else {
        dpoResult = { error: "unknown_op" };
      }
    } catch (dpoErr) {
      dpoResult = { error: String((dpoErr && dpoErr.message) || dpoErr) };
    }
    return ContentService.createTextOutput(JSON.stringify(dpoResult))
      .setMimeType(ContentService.MimeType.JSON);
  }
  // PROJECT END
...
function doGet(e) {
  ...
  // ── Fetch-transport GET API (iframe-free architecture) ──
  // GET fallback for the doPost fetch actions. Added 2026-07-09 after Google's
  // serving change was observed mangling POSTs to /exec (the deploy webhook got
  // "Script function not found: doPost" from a deployment that has doPost).
  // GET requests to /exec are the one transport verified working. The HTML layer
  // tries POST first and falls back to this route automatically.
  if (action === 'api') {
    ...
    try {
      ...
      } else if (apiOp === 'getAll') {
        // PROJECT: testauthgas1 data operations (GET variants)
        apiResult = { type: 'live-data', data: getAuthenticatedData(apiToken) };
      } else if (apiOp === 'writeCell') {
        apiResult = writeCell(apiToken, parseInt(e.parameter.row, 10), parseInt(e.parameter.col, 10), e.parameter.value || '');
      } else if (apiOp === 'addRow') {
        apiResult = addRow(apiToken, e.parameter.values || '[]');
      } else if (apiOp === 'deleteRow') {
        apiResult = deleteRow(apiToken, e.parameter.rowIndex);
      } else if (apiOp === 'presence') {
        apiResult = updatePresence(apiToken, (e.parameter.state || ''));
      } else if (apiOp === 'getAppUi') {
        // Hybrid architecture: session-gated app UI delivery (GET fallback)
        var uiCacheG = getEpochCache();
        if (!apiToken || !uiCacheG.get("session_" + apiToken)) {
          apiResult = { error: 'no_session' };
        } else {
          apiResult = getAppUiPayload_();
        }
      } else {
  ...
  var html = `
    ...
    <head>
      ...
      <style>
        ...
        #ld-conn-status {
          display: flex; align-items: center; gap: 5px; padding: 3px 9px; white-space: nowrap;
          background: #0d1117; border: 1px solid #30363d; border-radius: 20px; font-size: 11px; color: #8b949e;
        }
        #ld-conn-dot {
        ...
        #ld-conn-label { white-space: nowrap; color: #e6edf3; font-weight: 600; }
...
// ══════════════
// ══════════════
// PROJECT START — testauthgas1 hybrid app UI payload
// ══════════════
// The Live Data app UI lives HERE on the GAS layer and is delivered to the
// host page as session-gated content via dataOp getAppUi (hybrid architecture):
// never framed (immune to X-Frame-Options), never present in the public Pages
// HTML. The host shell injects css+js; the js builds its own markup and exposes
// window._fetchLdShow / window._fetchLdHide. Server calls use the host's
// _gasPost helper (fetch) — google.script.run does not exist outside the
// sandbox iframe. Keep this payload free of backticks and ${ sequences.
function getAppUiPayload_() {
  var css = `
    #live-data-app {
      position: fixed; top: 30px; left: 0; right: 0; bottom: 30px; z-index: 2;  /* 30px top/bottom bands (template default) */
      display: none; flex-direction: column;
      background: #0f1117; color: #e1e4e8; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
      overflow: hidden;
    }
    ...
    #ld-conn-dot { width: 8px; height: 8px; border-radius: 50%; background: #888; transition: background 0.3s; }
    #ld-conn-dot.live { background: #3fb950; }
    #ld-conn-dot.updating { background: #d29922; animation: ld-pulse 0.6s infinite alternate; }
    #ld-conn-dot.disconnected { background: #f85149; }
    #ld-conn-label { white-space: nowrap; color: #e6edf3; font-weight: 600; }
    #ld-countdown { font-size: 11px; color: #6e7681; min-width: 20px; text-align: right; }
    /* Viewer presence pill — exact template chip styling (mirrors .dt-presence) */
    #ld-presence { position: relative; display: inline-flex; align-items: center; gap: 5px; font-size: 11px; color: #8b949e; background: #0d1117; border: 1px solid #30363d; border-radius: 20px; padding: 3px 9px; white-space: nowrap; cursor: default; }
    #ld-pres-ico { font-size: 12px; }
    #ld-pres-count { color: #e6edf3; font-weight: 600; }
    #ld-pres-pop { position: absolute; top: 100%; left: 0; margin-top: 6px; min-width: 190px; max-height: 280px; overflow: auto; background: #161b22; border: 1px solid #30363d; border-radius: 8px; padding: 6px; z-index: 60; box-shadow: 0 6px 20px rgba(0,0,0,.45); display: none; }
    #ld-presence:hover #ld-pres-pop { display: block; }
    .ld-pres-row { display: flex; align-items: center; gap: 7px; padding: 4px 7px; border-radius: 5px; font-size: 12px; color: #c9d1d9; white-space: nowrap; }
    .ld-pres-dot { width: 8px; height: 8px; border-radius: 50%; flex: 0 0 auto; }
    .ld-pres-dot.active { background: #3fb950; }
    .ld-pres-dot.away { background: #d29922; }
    .ld-pres-you { color: #6e7681; }
    .ld-pres-empty { padding: 6px 8px; color: #6e7681; font-size: 11px; }
    @keyframes ld-pulse { from { opacity: 1; } to { opacity: 0.4; } }
    #ld-view-tabs { display: flex; background: #1a1b26; border-bottom: 1px solid #2a2b3d; flex-shrink: 0; }
    .ld-view-tab {
      padding: 10px 20px; font-size: 13px; font-weight: 600; color: #8b949e;
      cursor: pointer; border-bottom: 2px solid transparent; transition: all 0.2s;
      background: none; border-top: none; border-left: none; border-right: none;
    ...
    #ld-data-table td.editing input:focus { box-shadow: 0 0 0 2px rgba(88,166,255,0.3); }
    #ld-dashboard-view { flex: 1; overflow: auto; padding: 16px; display: none; }
    #ld-dashboard-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 16px; }
    .ld-dash-card {
      background: #1a1b26; border: 1px solid #2a2b3d; border-radius: 12px;
      padding: 20px; transition: border-color 0.2s;
    }
    ...
    .ld-card-changed { animation: ld-card-flash 1.5s ease-out; }
    #ld-empty-state { display: none; flex: 1; align-items: center; justify-content: center; color: #6e7681; font-size: 14px; text-align: center; padding: 40px; }
    #ld-data-table td, #ld-data-table th, .ld-view-tab { touch-action: manipulation; }
    #ld-delete-modal {
      display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0;
      z-index: 10000; background: rgba(0,0,0,0.6);
      align-items: center; justify-content: center;
...
`;
  var js = `
// ── Live Data app (fetch transport, iframe-free) ──
// Ported from the GAS-served UI on 2026-07-09 when Google began serving
// X-Frame-Options: SAMEORIGIN on framed /exec responses. Same UI, same server
// functions — but rendered by this page and driven via _gasPost(action=dataOp)
// instead of google.script.run inside the sandbox iframe.
(function() {
  // Inject the panel markup (hidden until _fetchLdShow activates it)
  var _ldMarkup = ''
    + '<div id="live-data-app">'
    +   '<div id="ld-header">'
    +     '<div class="left">'
    ...
    +         '<span id="ld-poll-countdown" style="font-size:11px;color:#6e7681;">--</span>'
    +       '</div>'
    +       '<div id="ld-presence" title="People currently viewing - green = active, amber = away (window not focused). Hover for the list."><span id="ld-pres-ico">👥</span><span id="ld-pres-count">–</span><div id="ld-pres-pop"></div></div>'
    +     '</div>'
    +     '<div class="right"></div>'
    +   '</div>'
    +   '<div id="ld-view-tabs">'
    ...
    +   '</div>'
    +   '<div id="ld-add-row-bar" style="display:none;flex-shrink:0;padding:8px 16px;background:#161b22;border-bottom:1px solid #2a2b3d;gap:8px;align-items:center;flex-wrap:wrap;">'
    +     '<input id="ld-add-col1" type="text" placeholder="Column 1" style="flex:1;min-width:80px;padding:6px 10px;font:13px/1 monospace;background:#0f1117;color:#c9d1d9;border:1px solid #2a2b3d;border-radius:6px;outline:none;">'
    +     '<input id="ld-add-col2" type="text" placeholder="Column 2" style="flex:1;min-width:80px;padding:6px 10px;font:13px/1 monospace;background:#0f1117;color:#c9d1d9;border:1px solid #2a2b3d;border-radius:6px;outline:none;">'
    +     '<input id="ld-add-col3" type="text" placeholder="Column 3" style="flex:1;min-width:80px;padding:6px 10px;font:13px/1 monospace;background:#0f1117;color:#c9d1d9;border:1px solid #2a2b3d;border-radius:6px;outline:none;">'
    +     '<input id="ld-add-col4" type="text" placeholder="Column 4" style="flex:1;min-width:80px;padding:6px 10px;font:13px/1 monospace;background:#0f1117;color:#c9d1d9;border:1px solid #2a2b3d;border-radius:6px;outline:none;">'
    +     '<button id="ld-add-row-btn" style="padding:6px 14px;background:#238636;color:#fff;border:none;border-radius:6px;font:13px/1.2 sans-serif;cursor:pointer;white-space:nowrap;">Add Row</button>'
    +   '</div>'
    +   '<div id="ld-table-view">'
    +     '<table id="ld-data-table"><thead><tr></tr></thead><tbody></tbody></table>'
    +   '</div>'
    +   '<div id="ld-dashboard-view"><div id="ld-dashboard-grid"></div></div>'
    +   '<div id="ld-empty-state">Waiting for data...</div>'
    + '</div>'
    + '<div id="ld-delete-modal">'
    +   '<div class="modal-box">'
    ...
    +   '</div>'
    + '</div>';
  document.body.insertAdjacentHTML('beforeend', _ldMarkup);
  ...
  var _addRowPendingIndex = undefined;
  var _deleteRowPending = null;
  var _updateAddRowPlaceholders = null;
  ...
  // Session token read at call time (fetch transport has no _sessionToken message)
  function _ldToken() { return loadSession().token || ''; }

  // Session-expiry detection on data-op responses — mirrors gas-session-invalid
  function _ldCheckSessionError(result) {
    var err = result && result.error ? String(result.error) : '';
    if (err.indexOf('SESSION_') === 0) {
      stopHeartbeat();
      stopCountdownTimers();
      clearSession();
      ...
      if (err === 'SESSION_EVICTED') msg = 'Your session was ended (signed in elsewhere). Please sign in again.';
      else if (err === 'SESSION_INTEGRITY_VIOLATION') msg = 'Session integrity check failed. Please sign in again.';
      showAuthWall(msg);
      return true;
    }
    return false;
  ...
  function ldStartStaleCheck() {
    ...
    _ldStaleTimer = setInterval(function() {
      if (_ldLastDataTs === 0) return;
      var age = Date.now() - _ldLastDataTs;
      if (age > LD_STALE_THRESHOLD) { ldSetConnectionStatus('disconnected'); }
      else { ldSetConnectionStatus('live'); }
    }, 1000);
  }
  ...
  function _executeDeleteRow(rowIndex, deletedRow) {
    ...
    _deleteRowPending = { index: rowIndex, row: deletedRow };
    _gasPost({ action: 'dataOp', op: 'deleteRow', token: _ldToken(), rowIndex: rowIndex })
      .then(function(result) {
        _deleteRowPending = null;
        if (_ldCheckSessionError(result)) return;
        if (result && result.liveData) _handleLiveData(result.liveData);
      })
      .catch(function(err) {
        console.error('Delete row failed:', err);
  ...
  function ldStartCellEdit(td, row, col, currentVal) {
    ...
    function commitEdit() {
      ...
      td.textContent = newVal;
      if (newVal !== currentVal) {
        _gasPost({ action: 'dataOp', op: 'writeCell', token: _ldToken(), row: row, col: col, value: newVal })
          .then(function(result) {
            if (_ldCheckSessionError(result)) return;
            if (result && result.liveData) _handleLiveData(result.liveData);
          })
  ...
  function ldRenderDashboardView(changedCells) {
    ...
    var sm = document.createElement('div');
    sm.className = 'card-meta';
    sm.textContent = 'Data via GAS CacheService · fetch transport (iframe-free)';
    stats.appendChild(sl);
    stats.appendChild(sv);
    stats.appendChild(sm);
    ldDashGrid.appendChild(stats);
  ...
  // Data poll — fetch transport (POST action=dataOp&op=getAll)
  var _dataPollInFlight = false;
  var _lastDataPollTick = 0;
  var DATA_POLL_INTERVAL = 15000;
  var _ldDataPollTimer = null;
  var _pollCountdownEl = document.getElementById('ld-poll-countdown');
  ...
  // Viewer presence: report own state every 30s, render the roster (template default).
  // Fetch transport - POST action=dataOp&op=presence (GET fallback: action=api&op=presence).
  ...
  function _presRender(viewers) {
    try {
      var cnt = document.getElementById('ld-pres-count');
      var pop = document.getElementById('ld-pres-pop');
      var n = viewers ? viewers.length : 0;
      if (cnt) cnt.textContent = n ? String(n) : '\u2013';
      if (!pop) return;
      if (!n) { pop.innerHTML = '<div class="ld-pres-empty">No one else viewing</div>'; return; }
      var html = '';
      for (var i = 0; i < viewers.length; i++) {
        var v = viewers[i] || {};
        var cls = (v.state === 'away') ? 'away' : 'active';
        var you = v.self ? ' <span class="ld-pres-you">(you)</span>' : '';
        var lbl = (v.state === 'away') ? ' \u2014 away' : '';
        html += '<div class="ld-pres-row"><span class="ld-pres-dot ' + cls + '"></span><span>' + _presEsc(v.name || v.email || '') + '</span>' + you + '<span class="ld-pres-you">' + lbl + '</span></div>';
      }
      pop.innerHTML = html;
    } catch(e) {}
  }
  function _presReport() {
    try {
      if (!_ldToken()) return;
      _gasPost({ action: 'dataOp', op: 'presence', token: _ldToken(), state: _presState() })
        .then(function(res) { _presRender(res && res.viewers); })
        .catch(function() {});
    } catch(e) {}
  }
  (function() {
  ...
  function _doDataPoll() {
    if (_dataPollInFlight) return;
    // Session gone (signed out / expired) — stop polling quietly
    if (!_ldToken()) { _stopLdDataPoll(); return; }
    _dataPollInFlight = true;
    _updatePollDisplay();
    _gasPost({ action: 'dataOp', op: 'getAll', token: _ldToken() })
      .then(function(result) {
        _dataPollInFlight = false;
        _lastDataPollTick = Date.now();
        _updatePollDisplay();
        ...
        if (payload) { try { _handleLiveData(payload); } catch(e) { console.error('Data poll error:', e); } }
        _ldDataPollTimer = setTimeout(_doDataPoll, DATA_POLL_INTERVAL);
      })
      .catch(function() {
        _dataPollInFlight = false;
        _lastDataPollTick = Date.now();
  ...
  function _startLdDataPoll() {
    if (_ldDataPollTimer) return;
    _lastDataPollTick = Date.now();
    _updatePollDisplay();
    // First poll fires immediately (no iframe-embedded initial data in fetch mode),
    // then chains via setTimeout so the countdown matches actual timing.
    _doDataPoll();
  }
  function _stopLdDataPoll() {
    if (_ldDataPollTimer) { clearTimeout(_ldDataPollTimer); _ldDataPollTimer = null; }
  }

  // Activation — called from showApp (fetch mode) with the auth context
  window._fetchLdShow = function(email, permissions) {
    if (ldApp) ldApp.classList.add('active');
    _ldCanEdit = permissions && permissions.indexOf('write') !== -1;
    var addRowBar = document.getElementById('ld-add-row-bar');
    if (addRowBar) addRowBar.style.display = _ldCanEdit ? 'flex' : 'none';
    if (_ldHeaders.length > 0) ldRenderTableView({});
    ldSetConnectionStatus('updating');
    ldStartStaleCheck();
    _startLdDataPoll();
    _presReport();   // fill the viewers pill immediately on sign-in (heartbeat continues every 30s)
  };
  window._fetchLdHide = function() {
    if (ldApp) ldApp.classList.remove('active');
    _stopLdDataPoll();
    if (_ldStaleTimer) { clearInterval(_ldStaleTimer); _ldStaleTimer = null; }
  };
  ...
  (function() {
    ...
    inputs.forEach(function(inp) {
      ...
      inp.addEventListener('blur', function() { this.style.borderColor = '#2a2b3d'; });
    });
    _updateAddRowPlaceholders = function(headers) {
      if (!headers) return;
      inputs.forEach(function(inp, i) {
        if (inp && headers[i]) inp.placeholder = String(headers[i]);
      });
    ...
    if (addRowBtn) {
      addRowBtn.addEventListener('click', function() {
        ...
        addRowBtn.style.cursor = 'wait';
        _addRowPending = true;
        _gasPost({ action: 'dataOp', op: 'addRow', token: _ldToken(), values: JSON.stringify(values) })
          .then(function(result) {
            var wasAdd = _addRowPending;
            _addRowPending = false;
            _addRowPendingIndex = undefined;
...
`;
  return { css: css, js: js, uiVersion: VERSION };
}
// ══════════════
// PROJECT END
// ══════════════
