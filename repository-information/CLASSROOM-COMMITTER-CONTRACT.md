...
- **Permitted edits inside the fence, and nothing else:**
  1. **Append** a new `clLesson<Name>_()` returning a strict-JSON lesson literal (schema v1, `CLASSROOM-SCHEMA.md`), or a new `clTrack<Name>_()` — **never a `type: scenario` literal** (C5, design D6: a scenario's beats were judged against the facts they were written from, and only a developer session re-judges them; **P13**)
...
- **A `type: scenario` lesson is never touched by the committer** (C5, design D6). It lives inside the content fence, so it is inside the write set by geography; this rule closes that gap by rule rather than by fence. There is no permitted revision: a run whose pin table finds a scenario's input moved reports it under `Needs the developer` and writes nothing — a refreshed record with un-re-judged beats is worse than a stale scenario wearing a "review due" chip. **P13** fails any diff that adds a scenario literal or changes an existing one in any byte.
...
| P13 | No `type: scenario` literal is added, changed in any byte, or removed base → head | §3.1 · §4.4 · C5 design D6 |
...
4. **`check-classroom-pipeline.py`** (§7): the thirteen assertions, each with at least one failing fixture
...
| 10.4 | The diff-aware checker | `scripts/check-classroom-pipeline.py`, P1–P13, `--base origin/main` against the working tree. `--selftest` runs **fifteen** fixtures: one **positive** (a well-formed pipeline commit, which must be clean) and one negative per assertion, each the positive with a single forbidden mutation applied — **two for P13**, since D6 forbids two distinct writes (authoring a scenario and revising one). The suite refuses to run if any of P1–P13 lacks a fixture | `scripts/check-classroom-pipeline.py` |
...
Two things C2b deliberately did **not** do. It did not soften any assertion to make the current repo pass: P1–P13 are written against the contract, and this developer commit fails P1 by design (it writes seven paths outside the committer's write set) — that is the checker working, not a defect. And it did not give the committer a way to record a `STAND-DOWN` or a `BLOCKED` run anywhere in the repo; §5's silence is preserved exactly.

### Settled at C5 session 1 (2026-09-18, v06.52r) — **in force**

`C5-SALES-SIMULATIONS-DESIGN.md` added a third lesson `type`, `"scenario"`, **inside** the content fence — so inside this contract's write set by geography — and closes that gap by rule rather than by fence: **the committer never authors and never revises a `type: scenario` lesson** (design D6). The three edits this note planned are made: §3.1 permitted edit 1 now says "never a `type: scenario` literal", §4.4 carries "a scenario literal is never touched by the committer", and §7 carries **P13 — scenario discipline**, with two negative fixtures (one that authors a scenario, one that revises one) taking `--selftest` from 13 to **15 fixtures, 0 failures**. No schema-version change accompanies it (design D2), so §5.1 step 4 is unaffected — which was the decisive reason against a bump, since a version mismatch would have `BLOCKED` every run from the moment it landed.

Two scenarios exist as of v06.52r (`scenario-storage-developers-and-ipps-objection`, `scenario-aidc-developers-and-landlords-objection`); the ledger of the planned fourteen is `CLASSROOM-CURRICULUM-PLAN.md` §11. A run that encounters one leaves it alone and reports it.

**Still open for the developer, and not changed by that session** (C5 design §12 item 7): §4.4's second bullet and the P5 row of `.claude/rules/classroom-app.md` both say `clStudyNext_` "walks registry order then track order". Measured, it walks **`clTracks_()` only** — finding (rr26) in `CLASSROOM-CURRICULUM-PLAN.md` §10.6. The drift is benign for C5 (a scenario in no track is unreachable by study-next with no code change) and is recorded rather than fixed, because this document is the developer's.
