...
**There is no `event:` prefix either — declined, not deferred, at X (2026-09-23; `NETWORK-EVENTS-DESIGN-PLAN.md` §3 D13 and §11).** The public Events registry (`live-site-pages/events-data/events.json`) is a public-kind source in principle, but a row carries dates, a venue and an audience list rather than mechanism, attendance is a Network signal that never crosses, and a tenth prefix costs more than this table and the `gateDigest` — a G7 resolution rule, the committer contract's count, and the P11 guard in `check-classroom-pipeline.py` that hard-codes the nine. The reopen conditions are in that §11 row; until one holds an `event:` ref is an unknown prefix and folds to deny exactly as `note:` does. There is never a `contact:` prefix.
...
- **`CL_DRILL_ACCOUNT_CAP` = 6000** state rows per account — **raised from 3,000 at v06.58r (2026-09-19, GAS v01.80g), the developer taking (rr58)** — tested against the account's **whole** row set: both decks share the tab and the check runs before the deck split. **At 3,000 it bound**: measured 2026-09-19 at v06.57r from the real ops, a contributor's mechanism deck is **3,139** (`lc` 253 + `lq` 280 + `gc` 275 + `gq` 162 + `sf` 766 + `ss` 1,403) and **3,453** with the roster deck's 314 on; an analyst's is 2,615 / 2,929 — so a contributor who had graded every mechanism card once was refused the last 139 first grades, and an opted-in one the last 453. At 6,000 the cap is a guard again at roughly 1.7× the largest deck measured; the deck grew 2,050 → 3,453 in the twelve days after K1, so re-measure it when the corpus grows. Past the cap a never-graded item's first grade returns `DRILL_FULL` — the card is still served, its schedule is not kept. The cap cannot stop a malformed client growing the tab, because grading re-authorises every id against the served pool (`ITEM_DENIED`); its remaining job is bounding the tab. `scripts/check-classroom-curriculum.py` §4 prints each tier's deck against the cap, with and without the roster deck, and `--strict` flags a deck the cap would refuse
