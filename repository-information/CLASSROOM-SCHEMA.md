...
- **`CL_DRILL_ACCOUNT_CAP` = 3000** state rows per account, tested against the account's **whole** row set — both decks share the tab and the check runs before the deck split — and **it binds today**: measured 2026-09-19 at v06.57r from the real ops, a contributor's mechanism deck is **3,139** (`lc` 253 + `lq` 280 + `gc` 275 + `gq` 162 + `sf` 766 + `ss` 1,403) and **3,453** with the roster deck's 314 on; an analyst's is 2,615 / 2,929. Past the cap a never-graded item's first grade returns `DRILL_FULL` — the card is still served, its schedule is not kept. The cap cannot stop a malformed client growing the tab, because grading re-authorises every id against the served pool (`ITEM_DENIED`); its remaining job is bounding the tab. **The constant is the developer's call and is unchanged**: the recommendation on record (curriculum plan §10.6 (rr58)) is 6,000
