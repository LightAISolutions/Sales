...
> **Planned, not built — a third lesson `type`, `"scenario"` (C5, designed 2026-09-18 at v06.51r, proposed for the developer's approval).** `C5-SALES-SIMULATIONS-DESIGN.md` §4 specifies it: an ordinary schema-v1 literal (**no `CL_LESSON_SCHEMA_VERSION` bump** — additive fields only, per rule 2 below) with a required five-field `scenario` block (`mode` · `seat` · `segment` · `counterparty` · `stage`), ten fixed section ids with three `quiz` beats, a stamp that must carry `guidance:landscape-<segment>-YYYY-MM` so the fold is always `guidance`, no `report:` input, no drill items, never a track member, never written by the unattended pipeline. When C5 session 1 lands, the `type` row above widens to `module · briefing · scenario`, the block's rows are copied in beneath it, and this pointer goes. Until then `scripts/check-classroom-content.py` rejects the value, which is correct.
