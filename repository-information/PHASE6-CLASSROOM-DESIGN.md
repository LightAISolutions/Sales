...
- **C1 — Learning core**: track/lesson schema (provenance-gated); first tracks assembled from the existing corpus (public content inline, guidance via deep-links); per-account track progress + rollup; study-next pointer
  > **Status: schema slice built 2026-09-01** (Classroom v01.01g) — lesson + track schema v1 and the provenance stamp are specified in `CLASSROOM-SCHEMA.md` (single source of truth), implemented in `Classroom.gs` (`CL_PROVENANCE_REF_KINDS`, `clStampKinds_()`, registries, per-tier index filtering, read-only `action=classroom` ops through `clRequireLesson_()`), and verified by `scripts/check-classroom-content.py`. Registries are empty by design; the first tracks, the renderer, progress, and study-next are the remaining C1 slices
