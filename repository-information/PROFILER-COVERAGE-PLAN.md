...
- **CHANGELOG capacity — rotation has already fired once and will fire again inside this program.** It stood at 92/100 at approval, hit 100/100 and rotated (oldest whole date group moved to `CHANGELOG-archive.md` with SHA enrichment), and sits at **96/100 as of v04.79r — 4 pushes of headroom**. The remaining program is ~13 push commits (see §9), so **rotation is due on the fourth from here**, which lands inside the Phase C dossier block (around C8 or C9), not in the guide backfill as previously estimated. Budget an extra ~10 minutes in whichever session crosses 100
