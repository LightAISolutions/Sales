...
| `seats` | object | yes (E3, 2026-09-22) | The developer's two selling seats, keyed `storage-seller` · `aidc-power-seller`, each `{ label, segments[], basis }` — `segments[]` the buyer segments that seat sells into (ids from `segments[].id`), `basis` the inventory line that supports the set (`C5-SALES-SIMULATIONS-DESIGN.md` §9). **Read at run time by Events' recommendation score** (`EVENTS-SCHEMA.md` §6 `segmentFit`: the union of both seats — design plan §12.6, both seats weigh equally), never copied there. A seat gains a segment when the inventory does; `Classroom.html`'s `CL_SEATS` carries the same two keys with their labels |
...
This changes no pin and writes no date — **G2 is untouched**, and re-pinning still requires a session that actually re-read the source. It changes only which movements are worth a developer's attention. The same 2026-09-19 measurement after the change: **18 stale, 37 additions-only** — and five `concepts` pins stayed stale, correctly, because one entry that day was rewritten rather than added. *(Corrected at v06.66r — (rr70): the rewritten entry is **`ups`**, whose definition broadened from "a battery-backed power stage" to "a power stage backed by stored energy — a battery string or a spinning flywheel". The `leakage-inductance` alias removal this paragraph originally cited did not happen: measured across all four `concepts` pin dates, removals are `none` at every one and `leakage-inductance` is an addition. The amendment's conclusion is unaffected; only the worked example was misattributed.)*

**Second amendment (2026-09-19, v06.67r — (rr71)): a DATED source's own revision date is a signal too, and a schema migration moves it without moving a claim.** The rule above qualifies the *undated* layers. The dated ones — `profile:<slug>` and `study:<slug>`, whose live date is the `lastUpdated` written in their own body — have the same defect one layer up: **a dossier's `lastUpdated` cannot distinguish a repaired citation from a revised figure.** Measured at v06.66r: the 2026-09-05/06 schema v6 → v7 wave moved dossiers' `lastUpdated` while changing nothing any lesson had drawn on — truncated `source` URLs were repaired, the new `via` / `project` typings were populated, and `policyExposure` blocks were written where the field had been `null` — and all eighteen pins the checker reported stale turned out to be non-contradictions that (rr70) had to disprove by hand.

So `source_revision_only()` qualifies that signal the same way, reading the source as it stood at the pin and clearing the move **only** when every difference is provably claim-free:

- **A metadata field** (`schemaVersion`, `lastUpdated`, `profileVersion`) — it carries no claim.
- **A field that was empty at the pin** (`null`, `""`, `[]`, `{}`, or absent) — there was nothing there for the lesson to draw on, so populating it is an addition, not a revision. This is what clears the `policyExposure` blocks and the `via` / `project` typings.
- **A citation repaired from a strict prefix of itself** — a `source`, `url`, `linkedin`, `photo` or `website` string that only grew at its tail was truncated and is now whole; it points at the same document.
- **A new entry in an identity-keyed list where a new entry cannot rewrite an existing claim** — `sources`, `relationships`, `policyExposure`, `decisionMakers`, `productsAndServices`, `technicalSpecs`. A new counterparty, regime, person, product line or spec block says nothing about the ones already there.

**Everything else leaves the pin STALE**, including additions that the registry rule would clear. `recentDevelopments` and `strategyRead` are deliberately **not** additive here, and a study guide's new `sections[]` are not either: *a development, a judgement or a section appended later can supersede one a lesson taught*, and clearing that is the false negative the whole freshness layer exists to prevent. Anything unprovable — an unparseable blob, an unresolvable commit, a pin git cannot parse, a shallow clone with no history at the pin — is **STALE**. As with the registry rule, the check can only ever remove a finding it has positively disproved.

**Why `graph:profiler-graph` gets NO such rule, and will not (2026-09-19, v06.68r — (rr72)).** The obvious third sibling — edges keyed on `(a, b)`, a new edge treated as additive because a new pair says nothing about an existing pair — **is unsafe, and the graph is the one layer where it is unsafe.** A lesson can quote an individual edge, and every such figure proved byte-stable; but a lesson can also quote the **corpus's own shape**, and one does: `reading-the-graph` taught *"At the last build there were 1,260 edges across the corpus"* — correct when authored, falsified by a later rebuild the same day, and 221 short by the time it was found. An additive edge rule would have cleared exactly that pin. **A derived aggregate's own size is a claim, so there is no cheap proof that a wholesale regeneration changed nothing**; a rule that also demanded the edge count hold would never fire, because the graph gains edges on nearly every `profiler <Company>` run. The graph's pins therefore stay in the stale list and are closed by reading, which is the correct cost for a file whose every build is a new document. The corollary for authors: **do not teach a corpus-wide count.** State the scale and point the reader at the file's own `built` snapshot — the lesson now does.

Measured 2026-09-19 after the change: **13 stale, 37 additions-only, 5 migration-only** (and **12** after (rr72)'s revision of `reading-the-graph` re-pinned its graph input). Five of the ten dated pins cleared (`voltagrid`, `proenergy`, `mainspring-energy`, `stack-infrastructure`, `on-energy` — pure migrations); five stayed (`enchanted-rock` appended a sentence to a spec band, `kiewit` re-typed a relationship, `bloom-energy` added three developments and a judgement, `vertiv` added twelve sections). (rr70) had already read all five by hand and found none of them contradicts anything taught — which is exactly the point: **the checker reports what it cannot disprove, and a human closes the rest.** Like the first amendment, this writes no pin and touches no lesson.
