...
| Field | Type | Required | Meaning |
|-------|------|----------|---------|
| `schemaVersion` | number | yes | Profile schema version — currently `5` (v5 adds `relationships[].context`; v4 added the normalized-KPI fields on `financials` periods/metrics; v3 added `relationships[]`; v2 added `recentDevelopments[]`, `strategyRead[]`, and the product depth fields). Older profiles remain valid; the renderer skips absent sections. Write new/revised profiles at v5 |
