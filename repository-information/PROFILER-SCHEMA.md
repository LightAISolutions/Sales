...
| `relationships[]` | object[] | no (v3) | `{ "slug", "type", "note", "source" }` — **curated links to other covered companies**, written at research/refresh time. `slug` must exist in the registry (links to non-covered companies are omitted, not invented); `type` ∈ `customer`, `supplier`, `partner`, `competitor`, `investor`, `other`; `note` is the one-line basis for the link (what connects them, in plain language); `source` is an optional label/URL when the basis is a specific sourced event. **When the field is absent, the app derives provisional links at render time** by scanning the dossier's own summary, developments, and judgments for other covered companies' names — derived links show their evidence sentence and are superseded the moment `relationships[]` is populated. Backfill opportunistically on each dossier's next revision per "Extending the schema"; a research pass should curate every company the dossier's own prose mentions, plus material links the prose implies (a customer named in an order win, a named competitor, a JV partner) |
...
An **industry report** generated by the **Profiler Report Command** (see `.claude/rules/profiler-app.md`): a point-in-time synthesis built **from covered dossiers only** — never fresh research — and rendered by the app's Reports view. Files live in `live-site-pages/profiler-data/reports/` alongside `reports-index.json`.
...
`scopeLabel` is a short human string ("12 grid-scale BESS players"); `companies`/`citations` are counts; `status` is `current` or `superseded` (set to `superseded` when a newer report names this one in its `supersedes`). Keep the index in sync with the report files — the check script reconciles it.
