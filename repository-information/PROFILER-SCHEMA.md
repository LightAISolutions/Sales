...
| `technicalSpecs[]` | object[] | no | `{ "product", "specs": [...], "notes" }` — flagship products only, concrete figures. **`specs[]` accepts two entry shapes** and the app renders both (v01.40w): a `{ "label", "value" }` pair, rendered as a two-column row, or a **plain string**, rendered as a full-width statement row. Prefer the labelled pair for new authoring — it reads better in tables and exports — but never invent a label just to fit the shape; a spec that is naturally one sentence stays a string. Entries with no text are dropped, a group with no renderable rows and no `notes` is dropped, and a profile whose groups all drop renders **no** specs section at all rather than an empty heading |
