...
- **Industry Guidance (C3 sessions 1–2, v01.11w/v01.21g)** — the guidance hub came home from Profiler. The nine `guidanceDoc*_()` module literals and `guidanceDocs_`/`guidanceIndex_`/`guidanceDoc_` now live in `Classroom.gs` **below the `// CONTENT END` fence**, so an unattended C2 pipeline run cannot write them (`CLASSROOM-COMMITTER-CONTRACT.md` §3); they are outside the pipeline's write set by construction rather than by convention. `handleGuidanceOp_` answers `?action=guidance&gop=index|doc` on the same cookie-less transport (POST with the GET api-route fallback) behind `clRequire_(sess,'guidance')` — admin and contributor, the tiers `GUIDANCE_ROLES` admits in Profiler, so the gate follows the content across apps. The page routes `#guidance`, `#guidance/<id>[/<section>]` and `#guidance-glossary`, rendering modules through the same `cl*` primitives lessons use, plus a lane-grouped library, a cross-module search and the unified glossary. Modules render **read-only** through C3 sessions 1–2: their per-account ticks still live in Profiler's `gd_progress:` store until session 3 migrates them in one shot, and a second live store would make that a merge. Module ids are permanent — Scraper's `guidance:<module-id>` seeds and every lesson's provenance stamp key on them. Profiler keeps serving the same modules until its slice is cut over; `scripts/check-guidance-parity.py` fails the build if the two copies diverge. **Session 2 added the two federation pieces.** The **Admin lens** (`clLoadAdminLens`/`clApplyAdminLens`) re-hosts here behind `clCan('reports')` — admin only, the same tier Profiler's `OV_ROLE_CAPS` grants it — and reads `reports/reports-index.json` and `reports/<id>.report.json` straight off public Pages under `CL_DATA_BASE`, so no server route was added for it; a non-admin issues no report request at all. Panels **append** into `#cl-<sectionId>` (two current reports can anchor the same section, and today two do), and an anchor a revision has since deleted lands before `#cl-glossary` with a note rather than being dropped. The **guidance peer route** `?action=guidancepeer&t=…&gpop=mentions` answers `guidanceMentions_()`, which moved here with the content it scans; it is token-gated on the `GUIDANCE_PEER_TOKEN` Script Property exactly as Scraper's `?action=corpus` is on `CORPUS_TOKEN` — a different pair of peers and a deliberately different secret — refused flat while unset, with **no** browser-facing door, because Profiler's `guidanceAllowed_` remains the single boundary deciding who may ask
