...
- **The roster deck — a SECOND, opt-in drill deck (K2, v01.15w/v01.73g).** The Mermaid graph above is unchanged: K2 adds no file, no node and no edge, only a second queue behind the ops the transport node already carries. `clDrillRosterItems_` enumerates `rc:<lessonId>:<dossierSlug>` — one card per row of a `segment-*` lesson's `the-players` table, **314 across the nineteen segment lessons** — from the lessons this session may read (`clLessonVisible_`), so the deck follows regeneration with no registry read at drill time and reaches **analyst and above**, since every segment lesson's stamp folds to `tracks`. It is **separate in four ways that matter to this picture**: its own op parameter (`cop=drill&deck=roster`, with an unrecognised `deck` refused rather than falling through), its own caps (`CL_ROSTER_SESSION_CAP` 20 / `CL_ROSTER_NEW_CAP` 10), its own daily new-card budget via `clDrillStateForDeck_` — the two decks share the `ClassroomDrill` and `ClassroomDrillLog` tabs and the id prefix separates the rows, so opting in never slows the deck already in use — and an **opt-in flag with its own key in the progress property store, default off**, deliberately NOT a member of the progress map, because `clProgressVisible_` filters that map to readable lesson and module ids and would drop a preference key on every read. **The mechanism queue never contains an `rc:` item and the roster queue never contains anything else**, asserted on counts in both directions in `scripts/check-classroom-content.py` rather than left as a comment. The two namespaces are also separated by pre-existing code: `CL_DRILL_ID_RE` requires a trailing `:<n>` index and an `rc:` id ends in a slug, so **0 of the 314 match it** and the roster grade path had to be written explicitly rather than inherited.
