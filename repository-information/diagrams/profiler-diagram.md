...
- **Industry Guidance ops (admin, v01.38w/v01.17g)** — the same cookie-less fetch transport carries `?action=guidance&gop=index|doc` (POST with GET api-route fallback). Server-side `handleGuidanceOp_` requires the `admin` permission before returning document-analysis module JSON; the page's `✦ Industry Guidance` masthead button (created only for admin sessions by the auth wall's `pass()`) opens a full-screen overlay that renders the module — prose, tables, pros/cons, timeline, bars, flashcards, quiz, ledger, glossary tooltips — entirely from the fetched JSON. Content lives in Profiler.gs (repo + GAS project only, never on public Pages)
