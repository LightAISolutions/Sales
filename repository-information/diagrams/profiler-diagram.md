...
- **Industry Guidance — migrated out (C3 session 3, v01.89w/v01.39g)** — the hub is Classroom's. The nine `guidanceDoc*_()` module literals, `guidanceDocs_`/`guidanceIndex_`/`guidanceDoc_` and the `gop=index|doc` ops they answered were copied to `Classroom.gs` at C3 session 1 and **deleted here at session 3**, together with the page's library, search, unified-glossary, Admin-lens and module-render code and `scripts/check-guidance-parity.py`, the guard that held the two copies byte-identical while both were live. What Profiler keeps is **one** link out: the dossier's `✦ Covered in guidance modules` chips, which `<a href>` to `Classroom.html#guidance/<id>`. The `✦ Industry Guidance` masthead slot briefly became an `<a href="Classroom.html#guidance">` in the same session and was then **removed entirely at v01.90w**, together with the `🎓 Classroom` cross-link beside it, on the developer's direction — the two apps no longer advertise each other in their mastheads. The stack went from five slots to three (`#ov-network-btn` 4px, `#ov-reports-btn` 44px, `#ov-signins-btn` 84px) and `#ov-header`'s reserved height from 194px to 114px. The `guidance` capability and its tier list did not move; `verify-profiler-roles.py` now reads that row off the chip line instead of a button, and its `classroom` column is retired. **The `gd-*` renderer did NOT go**: guidance was never its only caller — `gdRenderDoc` draws the public study guides (v01.74w), the report view builds on `gdTable`/`gdTimeline`/`gdBars`/`gdProsCons`, and the sign-in log uses `gdTable` and `gdCloseBtn`.
- **The two guidance ops that still answer here** — `?action=guidance&gop=mentions|progress|setprogress` on the same cookie-less fetch transport (POST with GET api-route fallback), all three behind `guidanceAllowed_` (the `admin` permission, or a tier in `GUIDANCE_ROLES` — currently `contributor`). `gop=mentions` is **relayed, not computed**: since session 2 `guidanceMentions_()` scans the module JSON where it now lives, so `guidanceMentionsProxy_()` makes a server-to-server `UrlFetchApp` call to Classroom's `?action=guidancepeer&gpop=mentions` on the shared `GUIDANCE_PEER_TOKEN` Script Property — the same shape as the Coverage panel's Scraper corpus proxy above and deliberately **not** the same secret; while the property is unset the op answers `not_configured` and the chip line renders nothing. `gop=progress`/`gop=setprogress` still write one Script Property per account (`gd_progress:<email>`, localStorage as the offline fallback, local-only ticks migrated up on first sync) but it is **no longer a guidance store**: session 3's one-shot `exportGuidanceProgress()` carried every module tick to Classroom's `cl_progress:<email>`, and what remains is `study-<slug>` and `dossier-<slug>` — so the write path validates by containment against the public registry only, and a module id arriving from a cached page is skipped.
