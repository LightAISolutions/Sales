#!/usr/bin/env node
/**
 * build-megmeet-sst-briefing-pdf.mjs — renders repository-information/study-prep/megmeet/megmeet-sst-briefing-print.html
 * to repository-information/study-prep/megmeet/MEGMEET-SST-BRIEFING.pdf.
 *
 * A copy of build-sst-primer-pdf.mjs with the source, the output, the DevTools
 * port and the proof-page prefix changed. Same mechanism: the pre-installed Chromium
 ...
 *
 * The briefing is a single-edition study document on the primer's skin, so this
 * stays deliberately small: one source, one output, plus the --png proof mode,
 * which the authoring session is required to actually look at before calling the
 * PDF done.
 *
 * Usage: node scripts/build-megmeet-sst-briefing-pdf.mjs [--png]
 */
...
const PORT = 9336;
const SRC = resolve(ROOT, 'repository-information/study-prep/megmeet/megmeet-sst-briefing-print.html');
const OUT = resolve(ROOT, 'repository-information/study-prep/megmeet/MEGMEET-SST-BRIEFING.pdf');
...
<div style="width:100%;font:7pt/1.2 Arial,Helvetica,sans-serif;color:#6d6758;padding:0 14mm;display:flex;justify-content:space-between;">
  <span>Solid-State Transformers, End to End — Megmeet onboarding briefing · Internal · Not investment advice</span>
...
<div style="width:100%;font:6.5pt/1.2 Arial,Helvetica,sans-serif;color:#a8a294;padding:0 14mm;text-align:right;letter-spacing:.14em;text-transform:uppercase;">
  Jon Yang · Sales Research · 23 September 2026
</div>`;
...
try {
  ...
  if (process.argv.includes('--png')) {
    ...
    for (let p = 0; p < shots; p++) {
      ...
      writeFileSync(`${scratch}/mmsst-p${String(p + 1).padStart(2, '0')}.png`, Buffer.from(shot.data, 'base64'));
    }
