...
const REGISTRY = { schemaVersion: 1, built: '2026-09-22T00:00:00Z', events: [
  ...
  ev('alpha-2026', 'Alpha Show 2026', 'Alpha Show', '2026-11-16', '2026-11-18', { audience: ['neoclouds'], relevance: 4, exhibitorListUrl: A2Z_ALPHA, speakersUrl: SPK_ALPHA }),   // starred
  ...
  ev('gamma-2026', 'Gamma Forum 2026', 'Gamma Forum', '2026-12-01', '2026-12-02', { audience: ['utilities', 'capital'], relevance: 5, exhibitorListUrl: ATTENDEE_LIST }),   // ranks first; unknown host
...
const ACCOUNTS = [
  { id: 'a-0000000000001', name: 'Fluence Energy', slug: 'fluence-energy', relationship: 'target', stage: 'shortlist' },
  { id: 'a-0000000000002', name: 'Sungrow', slug: 'sungrow', relationship: 'customer', stage: 'none' },
  { id: 'a-0000000000003', name: 'Bolt Supply', slug: '', relationship: 'supplier', stage: 'none' },
  { id: 'a-0000000000004', name: 'Acme Storage', slug: '', relationship: 'partner', stage: 'none' },
  { id: 'a-0000000000005', name: 'Tesla', slug: 'tesla', relationship: 'target', stage: 'prospecting' }
];
...
ACCOUNTS.forEach((a) => {
  ...
  Object.assign(row, { 'Account ID': a.id, 'Owner': OWNER, 'Name': a.name, 'Normalised Name': vm.runInContext('nwNormaliseCompany_', NW)(a.name), 'Profiler Slug': a.slug,
    'Relationship': a.relationship, 'Stage': a.stage, 'Segment IDs': '[]', 'Tags': '[]', 'Created At': TODAY, 'Updated At': TODAY });
...
ok(r.pages === 5 && r.pagesFailed === 1, 'sweep: five pages read, one failed (' + r.pages + ' / ' + r.pagesFailed + ')');
ok(r.feeds.length === 3 && r.feeds[0].key === 'prnewswire' && r.feeds[0].items === 5 && r.feeds[1].items === 0 && r.feeds[2].error === 'fetch_failed',
   'feeds: PR Newswire five items, Business Wire empty, GlobeNewswire unreachable and named (' + JSON.stringify(r.feeds) + ')');
...
ok(r.found === 8 && r.written === 8 && r.updated === 0 && r.rejected === 0 && !r.writeError, 'sweep: eight signals found and written through Network (' + JSON.stringify({ f: r.found, w: r.written, u: r.updated, rj: r.rejected }) + ')');
// Network's Signals tab — the real far side wrote them
let rows = sigRows();
ok(rows.length === 8 && rows.every((x) => /^s-[0-9a-z]{13}$/.test(col(x, 'Signal ID')) && col(x, 'Owner') === OWNER && col(x, 'Source') === 'events' && /^\d{4}-\d{2}-\d{2}T/.test(col(x, 'First Seen'))),
   'Network Signals: eight rows, s- ids, Source events, First Seen ISO');
const kinds = rows.map((x) => col(x, 'Kind') + ':' + col(x, 'Confidence')).sort().join(',');
ok(kinds === 'exhibitor:0.9,exhibitor:0.9,exhibitor:0.9,exhibitor:0.9,press-release:0.8,press-release:0.8,speaker:0.9,speaker:0.9', 'Network Signals: the three kinds with their confidences (' + kinds + ')');
...
ok(pageFail.length === 1 && pageFail[0].details.slug === 'omega-2026' && pageFail[0].details.source === 'exhibitors' && pageFail[0].details.status === 503, 'audit: exactly one page-failed row (omega, exhibitors, 503)');
...
ok(runAudit && runAudit.user === OWNER && runAudit.details.found === 8 && runAudit.details.written === 8 && Object.values(runAudit.details).every((v) => typeof v === 'number'), 'audit: the run row carries counts only, as the admin who pressed it');
...
ok(counters.urls.filter((u) => /prnewswire|businesswire|globenewswire/.test(u)).length === 3, 'fetches: each feed once per run, not once per event');
// the parked state for the panel
const parked = JSON.parse(props.EV_SIGNALS_LAST);
ok(parked.written === 8 && parked.events === 13 && parked.pagesFailed === 1 && !('results' in parked) && JSON.stringify(parked).toLowerCase().indexOf('fluence') < 0, 'last-run state: counts only, parked for the panel');
const state = call('evSignalsState_');
ok(state.installed === false && state.last.found === 8 && /Tuesday 06:00/.test(state.schedule), 'evSignalsState_: not installed, the last run\'s counts, the Tuesday schedule');

// ── 5. The re-run: written → updated, no duplicate row ────────────────────
ctx._evRegistryCache = null;
r = op(ADMIN, 'signalsnow');
ok(r.success && r.found === 8 && r.written === 0 && r.updated === 8 && sigRows().length === 8, 're-run: the same eight found, all updated, none written, still eight rows (' + JSON.stringify({ f: r.found, w: r.written, u: r.updated, rows: sigRows().length }) + ')');
ok(counters.audit.filter((a) => a.op === 'events_signals_page_failed').length === 2, 're-run: the failing page audited once per run');
...
ok(r.success === true && r.written === 0 && r.rejected.length === 1 && r.rejected[0].reason === 'linkedin_not_fetched' && sigRows().length === 8, 'manual (registrant-mail + a LinkedIn link): Network rejects it linkedin_not_fetched, no row');
...
ok(r.success === true && r.written === 1 && r.updated === 0 && r.rejected.length === 0 && sigRows().length === 9, 'manual (linkedin-manual + a LinkedIn link): accepted — the only LinkedIn entry (' + JSON.stringify(r) + ')');
...
ok(r.success && r.written === 0 && r.updated === 1 && sigRows().length === 9 && col(sigRows().find((x) => col(x, 'Kind') === 'linkedin-manual'), 'Confidence') === 0.9, 'manual again: the same link updates the row (confidence re-rated), never duplicates');
...
ok(read.success && read.signals.length === 2 && read.signals.some((s) => s.kind === 'speaker' && s.personName === 'Ann Lee' && s.personTitle === 'Director of Grid') && read.signals.some((s) => s.kind === 'exhibitor' && !('personName' in s)),
   'Network read leg: the person on the roster row, no person key on the exhibitor row');
...
// ── 10. Nothing live ──────────────────────────────────────────────────────
ok(counters.urls.every((u) => u.indexOf(SITE) === 0 || u.indexOf(NW_EXEC) === 0 || u === MYS_PROXY || u === A2Z_ALPHA || u === A2Z_OMEGA || u === SPK_REPLUS || u === SPK_ALPHA || u === SPK_BETA || /prnewswire|businesswire|globenewswire/.test(u)),
...
console.log('ALL CHECKS PASSED — the matcher is Network\'s key byte for byte; the Map Your Show, a2z, JSON-LD and HTML roster and RSS parsers read their fixtures; '
  ...
  + 'not_configured degrades; the ops are refused to a non-admin with zero fetches; installsignals is idempotent. Zero live calls.');
